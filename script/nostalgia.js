	...
	(function($)
	{
		...
		var Nostalgia=function(options,page,slide)
		{
						...
						$this.startPreloader.animate({width:0},1000,'easeOutQuint',function()
						{
							...
							$this.nostalgiaNavigationNameBox.bind('click',function()
							{
								if(parseInt($this.nostalgiaNavigationMenu.height())==0)
								{
									...
									$this.showNavigation(true,{complete:function()
									{
			...
			this.close=function(event)
			{
				...
				if(tab===false)
				{
					$this.moveNavigation(null,{complete:function()
					{
						$this.showNavigation(false,{complete:function() { $this.doEvent(event); }});
					}});
				}
				else
				{
			...
			this.moveNavigation=function(tabToOpen,event)
			{
				...
				if(tabToOpen=='left')
				{
					$this.nostalgiaNavigation.animate({right:'-300px'},{duration:500,complete:function() { $this.doEvent(event); }});
				}
				else if(tabToOpen=='right')
				{
					$this.nostalgiaNavigation.animate({right:'40px'},{duration:500,complete:function()  { $this.doEvent(event); }});
				}
				else
				{
					if($this.nostalgiaNavigation.css('right')!=='260px')
					{
						$this.showNavigation(false,{complete:function()
						{
							$this.nostalgiaNavigation.animate({opacity:0},{duration:500,complete:function()
							{
								$this.nostalgiaNavigation.css('right','');
								$this.nostalgiaNavigation.animate({opacity:1},{duration:500,complete:function()
								{
									$this.showSupersizedSliderControl(true);
									$this.showNavigationClickHereBox(true);
            ...
            {
                $this.scrollbar=$('#nostalgia-tab-content-scroll').jScrollPane({maintainPosition:false,autoReinitialise:true}).data('jsp');
            };
